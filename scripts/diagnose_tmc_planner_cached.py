...
PLANNER_TIMING_PATH = Path(
    os.environ[
        "TMC_PLANNER_TIMING_OUTPUT"
    ]
)
...
def cached_exact_planner(
    ...
    **kwargs,
    ...
    """Exact planner with per-call memoization only."""
...
def timed_continuation_planner(
    *args,
    **kwargs,
):
    start_ns = (
...
evaluator.find_optimal_continuation_mapping = (
    timed_continuation_planner
)


def save_planner_timings():
    PLANNER_TIMING_PATH.parent.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    print(
        f"Saved planner timings: "
        f"{PLANNER_TIMING_PATH}",
        flush=True,
    )


atexit.register(
    save_planner_timings
)
